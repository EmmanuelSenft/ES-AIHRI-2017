...
%be accepted in human society robots have to learn how to interact with humans.
    ...
    world. Actions executed by the robot have impacts on humans, and as such
    ...
    interact with humans. As such we argue that
    ...
    allowing a safer online learning with Reinforcement Learning and use
...
select actions appropriated to the current space. However,
 parts of the state can be irrelevant to the current goal or current state and
 are not required
...
of go~\cite{silver2016mastering}. Even where a virtual environment is available,
but especially when it is not, human knowledge is required at many steps of the
design of the algorithm or the learning phase to achieve an efficient action
policy.
...
supervised learning from go masters' games to learn a decent policy and then
used deep learning, self play and tree search to achieve super-human
capabilities and beating human best players.
...
actions the robot is about to execute and can cancel an action about to be
executed, let it be
...
use of any kind of generalisation, it was not applicable to
environments with a continuous or high dimensional space or undeterministic
transition from one state to another, such as interactions with humans.
...
datapoints to converge toward a good function approximator. However, 
 in many application, especially in HRI, these amounts of data are not
obtainable due to time or unconsistency of human reactions, so methods to
generalise with a lower number of inputs are required.
...
When the agent execute an action $a$ in state $s$, it receives the reward $r$
...
An example of application yet to be tested is robot for teaching children.
...
and depending of the representation of the actions on the algorithm side, the
